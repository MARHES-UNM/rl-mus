...
def train(args):
    ...
    args.config["env_config"]["stp_penalty"] = tune.grid_search([0.5, 1.0, 1.5, 0.25])
    args.config["env_config"]["tgt_reward"] = tune.grid_search([20])
    args.config["env_config"]["beta"] = tune.grid_search([0.3])
    args.config["env_config"]["beta_vel"] = tune.grid_search([0.1])
